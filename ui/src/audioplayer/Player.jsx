...
const Player = () => {
  ...
  const preloadOpenListForUpcomingTrack = useCallback(
    (info) => {
      if (info?.ended || info?.isRadio) {
        return
      }
      ...
      const next = playerState.queue[currentIdx + 1]
      if (!next || next.isRadio) {
        return
      }
  ...
  const onAudioProgress = useCallback(
    (info) => {
      ...
      preloadOpenListForUpcomingTrack(info)
    },
    [preloadOpenListForUpcomingTrack],
  )
